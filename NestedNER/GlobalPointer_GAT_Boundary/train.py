...
if config["logger"] == "wandb" and config["run_type"] == "train":
    wandb.init(project="GlobalPointer_GAT_Boundary_" + config["exp_name"], config=hyper_parameters)
...
def train_step(batch_train, model, optimizer, criterion):
    batch_samples, batch_input_ids, batch_attention_mask, batch_token_type_ids, batch_labels, batch_span_features = batch_train
    ...
    logits = model(batch_input_ids, batch_attention_mask, batch_token_type_ids, span_features_batch=batch_span_features)
...
def valid_step(batch_valid, model):
    # 解包数据，增加 span_features
    batch_samples, batch_input_ids, batch_attention_mask, batch_token_type_ids, batch_labels, batch_span_features = batch_valid
    batch_input_ids, batch_attention_mask, batch_token_type_ids, batch_labels = (
        batch_input_ids.to(device),
        batch_attention_mask.to(device),
        batch_token_type_ids.to(device),
